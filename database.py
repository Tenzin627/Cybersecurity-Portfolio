...
All queries use parameterized SQL (the "?" placeholders below) so user
input is never concatenated directly into a SQL statement.
"""
...
def insert_message(name, email, message):
    """Insert a contact form submission using a parameterized query."""
    connection = get_db_connection()
    connection.execute(
